...
from weights.clip_param_keys import clip_param_keys
...
class VideoCLIP(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        if optimizer == 'adam':
            ...
            optimizer = torch.optim.AdamW(self.parameters(), lr=self.lr, eps=1e-6, betas=(0.9, 0.98))
        else:
...
if __name__ == "__main__":
    ...
    dataset = AnimalKingdomDataset(data_dir, split="train")
    dataset.produce_prompt_embedding(model.clip)
    model.set_text_feats(dataset.text_features)
    val_loader = utils.data.DataLoader(dataset, batch_size=4, shuffle=False) # TODO: DEBUG num_workers=4
    ...
    # plt.savefig(os.path.join("temp", "video_logits.png"))
    # plt.close()
    ...
    # print(val_loader[0])
    # print(val_loader[0].keys())

    # df_action = dataset.df_action
    # ret = dataset[0]
    # print(ret['video_fp'])
    # print(ret['video'][0].shape)

    # # prompts = ['a dog is running', 'a cat is running', 'a bird is on the lake']
    # # text_inputs = tokenize(prompts)
    # # text_feats = model.clip.encode_text(text_inputs)

    # video_feats = model([d['video'] for d in dataset[:4]])

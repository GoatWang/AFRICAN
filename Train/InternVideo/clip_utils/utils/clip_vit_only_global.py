...
class ResidualAttentionBlock(nn.Module): # TODO: understand this
    def __init__(
...
class Extractor(nn.Module):
    def __init__(
        ...
        """
        d_model: The dimensionality of the model's input and output features.
        n_head: The number of attention heads to use in the self-attention mechanism.
        attn_mask: An optional attention mask to apply during the attention calculation.
        mlp_factor: A factor to determine the dimensionality of the intermediate MLP layer.
        dropout: The dropout probability to apply within the MLP.
        drop_path: The drop path probability for drop path regularization.
        """
        super().__init__()
...
class Transformer(nn.Module):
    ...
    self.transformer = Transformer(
        ...
        layers, # 24
        heads, # 16
        backbone_drop_path_rate=backbone_drop_path_rate, # 0.0
        use_checkpoint=use_checkpoint, 
        checkpoint_num=checkpoint_num, 
        t_size=t_size, # num_frames=8
        return_list=return_list, # [20, 21, 22, 23] (ViT-L-14.pt)
        n_layers=n_layers, # 4
        n_dim=n_dim, # 1024
        n_head=n_head, # 16
    ...
    def __init__(
        ...
        super().__init__()
        self.T = t_size # num_frames
        self.return_list = return_list # [20, 21, 22, 23] (ViT-L-14.pt)
        # Backbone
        b_dpr = [x.item() for x in torch.linspace(0, backbone_drop_path_rate, layers)] # all 0.0
        ...
        # Extractor: perform the self-attention mechanism 
    ...
    def forward(self, x, mode='video', return_all_feats=False):
        ...
        if mode == 'video':
            ...
            T_down = 1
        L, NT, C = x.shape # 257, 8, 1024
        N = NT // T_down # n_batches  
        H = W = int((L - 1) ** 0.5)
        cls_token = self.temporal_cls_token.repeat(1, N, 1) # generate 1024 dim vec for each batch (1, *, 1024)
        ...
        for i, resblock in enumerate(self.resblocks):
            ...
            if i in self.return_list: # [20, 21, 22, 23] (ViT-L-14.pt) => for outputing the features of the last 4 layers
                ...
                # dpe
                _, tmp_feats = tmp_x[:1], tmp_x[1:].clone() # 1, 1, 8, 1024 & 256, 1, 8, 1024
                tmp_feats = tmp_feats.permute(1, 3, 2, 0).reshape(N, C, T_down, H, W) # 1, 1024, 8, 16, 16
                # self.dpe[j](tmp_feats).shape == 1024, 8, 16, 16
                tmp_feats = self.dpe[j](tmp_feats).view(N, C, T_down, L - 1).permute(3, 0, 2, 1) # 1, 1024, 8, 256 => 256, 1, 8, 1024
                tmp_x[1:] = tmp_x[1:] + tmp_feats # identity mapping
                # enhancer: perform attention only on the class token
                tmp_x = tmp_x.permute(2, 0, 1, 3).flatten(0, 1)  # T * L, N, C  # 8*256, 1, 1024
                cls_token = self.dec[j](cls_token, tmp_x)

        weight = self.sigmoid(self.balance) # 1024
        residual = x.view(L, N, T_down, C)[0].mean(1)  # L, N, T, C # 257, 1, 8, 1024
        # return self.proj((1 - weight) * cls_token[0, :, :] + weight * residual)
        feats = (1 - weight) * cls_token[0, :, :] + weight * residual # the weighted sum of cls_token and residual (the first element of x mean along the batch dim)
        if return_all_feats:
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, mode='video', return_all_feats=False):
        ...
        out = self.transformer(x, mode=mode, return_all_feats=return_all_feats) # 1, 1024
        return out
...
def load_state_dict(model, state_dict):
    """
    Load original CLIP weights into this 3D video model.
    """
...
def vit_only_global_l14( # ViT-L-14.pt do this
    pretrained=True, use_checkpoint=False, checkpoint_num=[0],
    t_size=16, backbone_drop_path_rate=0., 
...
if __name__ == '__main__':
    ...
    model(torch.rand(1, 3, num_frames, 224, 224))

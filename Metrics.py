...
def set_metrics(pl_module): # _loss_names({"vtm": 1, "mlm": 1})
    set_split_metrics(pl_module, "train", pl_module.hparams.config["loss_names"])
    if len(pl_module.hparams.config["val_datasets"]) == 0: # if not defined, then use the same as train

...
class VideoIntern(nn.Module):
    def __init__(
        ...
        self.visual = utils.__dict__[backbone]( # vit_only_global_l14
            pretrained=False, # Do not use pretrained model
            t_size=t_size, # num_frames: 8
            mlp_dropout=mlp_dropout, # [0.5, 0.5, 0.5, 0.5]
            cls_dropout=cls_dropout, # 0.5
            n_dim=n_dim, # 1024
            n_head=n_head, # 16
            return_list=return_list, # [20, 21, 22, 23] (for vit_only_global_l14)
            drop_path_rate=drop_path_rate,  # 0.0
            backbone_drop_path_rate=drop_path_rate, # 0.0
            use_checkpoint=use_checkpoint, # False
            checkpoint_num=checkpoint_num, # [0, 0, 0]
        )

        self.visual_ln_post = nn.LayerNorm(n_dim)
        scale = n_dim**-0.5
        self.visual_proj = nn.Parameter(scale * torch.randn(n_dim, embed_dim)) # 1024, 768
...
def build_model(
    state_dict: dict, # static_dict of clip model
    n_layers=4, 
    n_dim=768,
    n_head=12,
    mlp_factor=4.0,
    ...
    # Jeremy Comment: Use Clip model to init hyperparameters
    ...
    grid_size = round((state_dict["visual.positional_embedding"].shape[0] - 1) ** 0.5) # 16 (state_dict["visual.positional_embedding"].shape[0] == 257)
    ...
    transformer_heads = transformer_width // 64 # 12 heads, 64 dimension for each head
    transformer_layers = len(
        set(
            k.split(".")[2]
            for k in state_dict
            if k.startswith(f"transformer.resblocks") # TODO: check the state_dict
        )
    )

    try:
        vision_width = state_dict["visual_proj"].shape[0] # for only vision transformer: (1024, 768)
    except:
        vision_width = state_dict["visual.proj"].shape[0] # for only vision transformer: (1024, 768)
    n_dim = vision_width
    if vision_width == 768:
        backbone = "vit_only_global_b16"
        n_head = 12
        return_list = [8, 9, 10, 11]
    elif vision_width == 1024: # ViT-L-14.pt do this
